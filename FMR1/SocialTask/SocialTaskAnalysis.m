...
%%
%
% PREPROCESS BEHAVIOR DATA (DIO ONLY HERE)
%
...
for i=1:length(ratinfo)
    ...
    % add half for the second run that day
    if ratinfo(i).runnum>6
        ratinfo(i).datenum=ratinfo(i).datenum+.5;
    end
    
    % and add elapsed time for each run (rat 2 gets put on the maze second)
    ratinfo(i).duration=ratinfo(i).ratsamples{2}.end(end)-ratinfo(i).ratsamples{2}.start(1);
end
%%
%
%
...
% first calculate the dwell times

for i=1:length(ratinfo)

    rat1dwells=[];
    % for each arm visit we can distill into dwell times
    % first method- dont include transitions, so its last unpoke to this
    % poke
    if fulldwell==0
        armMat=ratinfo(i).ratsamples{1};
        firstPoke=find(armMat.thiswell~=armMat.lastwell);
        lastPoke=firstPoke(2:end)-1;
        % now grab each firstpoke in time, last poke out time and compare
        pokeTime=[nan nan]; % poke duration and its condition(1 rew, 0 not)
        for pk=1:length(firstPoke)
            pokeTime(pk)=
