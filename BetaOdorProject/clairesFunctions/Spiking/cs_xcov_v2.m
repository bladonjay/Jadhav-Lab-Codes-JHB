        ...
        for p = 1:size(pairInds,1)
            ...
            for ep = eps'
                ...
                if ~isempty(spikes{day}{ep}{cCA1(1)}{cCA1(2)}.data) && ~isempty(spikes{day}{ep}{cPFC(1)}{cPFC(2)}.data)
                    ...
                    T = [T + sum(npwins(:,2)-npwins(:,1))];
                end
            end
            
            nspikes1 = length(spikesCA1);
            nspikes2 = length(spikesPFC);
                
            if nspikes1 >=50 && nspikes2 >= 50
                [timebase, corr_raw, corr_sm] = spiketrainxcorr(spikesCA1,spikesPFC,bin,tmax,sw1);
                nspikes1 = length(spikesCA1);
                nspikes2 = length(spikesPFC);
                
                % Expected probability
                p1 = nspikes1/T; p2 = nspikes2/T; % Fir rate in Hz
                exp_p = p1*p2; % per sec
    
                % Crosscov
                crosscov = (corr_raw ./ (bin*T))-exp_p;
               
                % Convert to Z-score
                factor = sqrt((bin*T) / exp_p);
                Zcrosscov = crosscov .* (factor);
                
                allpairs = [allpairs; Zcrosscov];
%                 %%%
%                 corr_norm = corr_sm;
%                 corr = corr_raw;
            end
...
save([topDir,'AnalysesAcrossAnimals\xcov_',filetag],'allpairs','timebase')