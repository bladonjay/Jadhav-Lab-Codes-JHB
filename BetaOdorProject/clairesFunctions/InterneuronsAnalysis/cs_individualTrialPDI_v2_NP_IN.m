...
load([topDir, 'AnalysesAcrossAnimals\significantPDI_NP_IN.mat'])
...
for a = 1:length(animals)
    ...
    %load cells
    load([topDir, 'AnalysesAcrossAnimals\',celltype,'Int_', region, '.mat'])
    
    try
        eval(['allcells = ',celltype,'Int;']);
    catch
        eval(['allcells = ',celltype,'int;']);
    end
...
%                 close all
            ...
            %% Save 
                save([animDir,animal,'trialSigPDI_NP_IN_',trialstr,region,daystr,'.mat'],'trialSigPDI')
                clear trialSigPDI
        end
 ...
 cs_PDIlfpCoherenceCorrelation_sextiles_IN
cs_PDIBehaviorCorrelation_IN
