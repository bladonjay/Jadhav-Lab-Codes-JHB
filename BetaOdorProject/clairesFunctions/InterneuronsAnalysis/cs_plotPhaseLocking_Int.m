...
for cr = 1:length(regions)
    ...
    load([topDir, 'AnalysesAcrossAnimals\PhaseLocking\plINs_',freq,'_',region,'-',region,'.mat']);
    ...
    for a = 1:length(aninds)
        ...
        load([animDir,'PhaseLocking\Interneurons\',animal,'phaselock_',freq,'_',region,'-',region,'.mat'])
        ...
        for c = 1:size(ancells,1)
            ...
            %% Save Figure
            figtitle = [freq,'_',region,'to',region,'_',num2str(anind),'_',num2str(cell(1)),'_',num2str(cell(2)),'_',num2str(cell(3))];
            newfigfile = [figDir,'Interneurons\PhaseLockCells\',figtitle];
