        ...
        for a = 1:length(animals)
            ...
            for day = days'
                ...
                for ep = eps'
                    ...
                    c = mean(data(isExcluded(times, trialtimes)));
...
%                     coh = [coh,bp];
                ...
                coh_all = [coh_all; mean(coh)];
                decisionTime_all = [decisionTime_all; mean(allTrials(:,2)-allTrials(:,1))];
...
%                 decisionTime_high = [decisionTime_high; exitTimes];
        ...
        subplot(3,2,fignum)
        plot(decisionTime_all, coh_all,'k.')
        hold on
        fit = polyfit(decisionTime_all, coh_all,1);
        plot([0.6 1.1], polyval(fit,[0.6 1.1]))
        

       
        ylabel([freq, ' Coherence'])
        xlabel(['Decision Latency (seconds)'])
        text(0.8,max(coh_all),['R = ',num2str(round(R,2,'significant')),newline,'p = ',num2str(round(p,2,'significant'))  ])
        box off
...
figfile = [figDir, 'EEG\Coherence-DecisionTime_Sessions'];
print('-djpeg',figfile)
print('-dpdf',figfile)