%cs_betaPowerPerformance_v4

%compare beta power between correct and incorrect trials
%uses zscored beta power compared to baseline (1s before odor)

%take average power on correct/incorrect over entire session, directly
%compare these two distributions so no need to downsample/bootstrap. 
...
for r = 1:length(regions)
    ...
    for a = 1:length(animals)
        ...
        for day = days'
            ...
            for ep = epochs'
                ...
                mncorrect = mean(correctbeta);
                mnincorrect = mean(incorrectbeta);
                C = [C;mncorrect];
                I = [I;mnincorrect];
    ...
    save([topDir,'AnalysesAcrossAnimals\betaPowerData_sessions'],'betaPowerData');
end
figure
hold on
...
figfile = [figDir, 'Behavior\betaPower_performance_sessions'];
