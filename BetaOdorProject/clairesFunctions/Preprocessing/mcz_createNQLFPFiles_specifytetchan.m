function mcz_createNQLFPFiles_specifytetchan(rawDir, dataDir,animID,sessionNum,tet,ch)
%CS- modified 1/21/20
%same as mcz_createNQLFPFiles, except that there are two additional inputs:
    %tet = tetrode to exctract
    %ch = channel for that tetrode. Must have extracted this channel already,
        %and have a dat file for it in the LFP folder. Change this in the config
        %file
%use for thermocouple data- need to subtract channels from each other to
%get a smooth signal, so must extract both channels. 
...
cd(targetFolder);
datFiles = dir(['*.LFP_nt',num2str(tet),'ch',num2str(ch),'.dat']);
...
for datFileInd = 1:length(datFiles)
    ...
    for e = 1:size(epochList,1)
        ...
        cd(dataDir);
        %mkdir EEG
        save([dataDir,'EEG_TC/',animID,'eeg',sessionString,'-',epochString,'-',nTrodeString,'ch',channelString,'.mat'],'-v6','eeg');
        cd(lfpdir);
    end
end
cd(currDir);    
