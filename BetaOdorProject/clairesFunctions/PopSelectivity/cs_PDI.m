...
%% Plot
    ...
    %PDI = smoothdata(PDI);
    plot(bins, PDI, 'k-','LineWidth',3);
    
    maximums(r) = max(PDI);
    
%     shuffmean = smoothdata(shuffmean);
%     prc05 = smoothdata(prc05);
%     prc95 = smoothdata(prc95);
    
    %shadedErrorBar(bins(1:end-1),shuffmean,[prc95; prc05],[],1)
    patch([bins, fliplr(bins)], [shuffmean-prc05,fliplr(shuffmean)+fliplr(prc95)], 'k','FaceAlpha',0.2,'EdgeColor','none')
...
save([topDir, 'AnalysesAcrossAnimals\significantPDI.mat'], 'significantPDI');
save([topDir, 'AnalysesAcrossAnimals\PV.mat'], 'PV');
...
for r = 1:length(regions)
    ...
    figtitle = ['4c_EnsemblePDI_',region];
