...
region = regions{r};
    ...
    for a = 1:length(animals)
        ...
        cellfilter = ['((strcmp($type, ''pyr'')) && (isequal($area,''',region, ... 
            ''')) && (strcmp($selectivity, ''novelLeftSelective_postlearn'') || strcmp($selectivity, ''novelRightSelective_postlearn'')))'];
...
%     [cmap]=buildcmap('rkg'); 
    ...
    %save
       figfile = [figDir,'IgarashiReplication\4a_OdorSelectivity_novel_',region,'_Correct'];
...
%     i_smoothed = zeros(length(validinds));
    ...
    %save
       figfile = [figDir,'IgarashiReplication\4a_OdorSelectivity_novel_',region,'_Incorrect'];
       %saveas(gcf,figfile,'fig');
       print('-dpdf', figfile);
       print('-djpeg',figfile);
    ...
    %save
       figfile = [figDir,'IgarashiReplication\4a_OdorSelectivity_novel_',region,'_SICorrelation'];
       %saveas(gcf,figfile,'fig');
       print('-dpdf', figfile);
       print('-djpeg',figfile);
       
       
     selectivityData.psth_correct = smoothed;
     selectivityData.psth_incorrect = i_smoothed;
     selectivityData.SI_correct = corr;
     selectivityData.SI_incorrect = incorr;
     selectivityData.cellinds = cellinds;
     selectivityData.win = [-(win(1)):binsize:win(2)];
     save([topDir,'AnalysesAcrossAnimals\selectivityData_novel_',region],'selectivityData');
