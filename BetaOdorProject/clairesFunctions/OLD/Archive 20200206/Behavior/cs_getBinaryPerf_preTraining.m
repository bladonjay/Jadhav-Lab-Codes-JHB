function [BinaryPerf,triggers] = cs_getBinaryPerf_preTraining(filename)
%topDir should be top Expt folder, WITH trailing slash

%This function was written for getting DIO triggers from statescript instead of DIO files. Written to get around DIO noise from Trodes 1.6.2 with
%headstages with sensors, should be fixed in newer trodes versions. not recommended for normal use (use cs_getOdorTriggers.m instead, uses DIO.mat files). 
...
%         case 'epochtype'
%             epochtype = varargin{option+1};
%     end
% end
% 
% animDir = ['D:\OdorPlaceAssociation\',animal,'Expt\PreTraining\'];
% cd(animDir)
% dayfolders = dir() ;
...
%         runEps = {};
%         day = days{d};
%         tmp = strfind(dayfolders, day);
%         dayfolderind = find(not(cellfun('isempty', tmp)));
%         cd(dayfolders{dayfolderind})
%         ssfiles = dir('*.stateScriptLog');
%         for i = 1:length(ssfiles)
%             epochname = ssfiles(i).name;
%             %s = strfind(epochname,'Sleep');
%             k = strfind(epochname,epochtype);
%             if ~isempty(k) 
...
%         stateScriptStruct = stateScriptStruct(want_idx);
        ...
        %Change these for novel odor solenoids
