            ...
            for c = 1:size(daycells,1)
                ...
                for ep = 1:length(epochs)
                    ...
                    if isa(spikes{day}{epoch}{cell(1)},'cell')
                        if isa(spikes{day}{epoch}{cell(1)}{cell(2)},'struct') && ~isempty(spikes{day}{epoch}{cell(1)}{cell(2)}.data)
...
%                             tmp = tmp(~isExcluded(tmp, excludePos));
                ...
                [occupancy, xticks, yticks] = hist2(allpos_l(:,1), allpos_l(:,2), binx, biny);
                 ...
                 if ~isempty(allspikes_l)
                    ...
                    z = find(smoothedocc <= threshocc);
                    plotspikerate = spikerate;
                    smoothedspikerate_l = filter2(g,plotspikerate);
                    smoothedspikerate_l(z) = -1;
%                     subplot(1,2,1)
%                     imagesc(smoothedspikerate);
                ...
                if ~isempty(allspikes_r)
                    ...
                    z = find(smoothedocc <= threshocc);
                    plotspikerate = spikerate;
                    smoothedspikerate_r = filter2(g,plotspikerate);
                    smoothedspikerate_r(z) = -1;
                %Fix size of matricies to plot together
                    fields = {smoothedspikerate_l,smoothedspikerate_r};
                    [nrows, ~] = cellfun(@size, fields);
                    height = max(nrows);
                    ...
                    smoothedspikerate = cell2mat(fields);
                    
                    if max(smoothedspikerate(:)) > 0 
                    imagesc(smoothedspikerate);
                    colorbar
                    titlestr = ['Cell ', num2str(cellnum), ' SI = ',num2str(si)];
                    title(titlestr);
                    figfile = [figDir, 'PlaceFields\TrajFields\',animal, '_', num2str(day), '-',num2str(cell(1)),'-',num2str(cell(2))];
