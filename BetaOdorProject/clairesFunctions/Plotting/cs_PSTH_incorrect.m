...
%% Params
cellstoplot = [3 1 29 5]; %[animal, day, tet, cell]
...
for c = 1:size(cellstoplot,1)
    ...
    daystr = getTwoDigitNumber(day);
     ...
     for ep = 1:length(runeps)
              ...
              [~, ~, incorrect_left, incorrect_right] = cs_getSpecificTrialTypeInds(odorTriggers{day}{epoch});
              lefttrigs = [lefttrigs; alltrigs(incorrect_left)];
              righttrigs = [righttrigs; alltrigs(incorrect_right)];
     end
     ...
     allspikes = [];
      for ep = 1:length(runeps)
       ...
       for t = 1:length(righttrigs)
        ...
        if savefig == 1
            figtitle = ['PSTH_',region,'_',animal,'_',num2str(cindex(2)),'_',num2str(cindex(3)),'_',num2str(cindex(4)),'_incorrect'];
