...
for a = 1:length(animals)
    ...
    for d = days'
        ...
        rewards = loaddatastruct(animDir, animal, 'rewards',d);
        for ep = eps'
            try
                specfull = spec{d}{ep}.Spec';
            catch
                specfull = spec{d}{ep}.Coh';
            end
            eptime = spec{d}{ep}.time;
            trigs = sort([rewards{d}{ep}.leftWindows(:,2); rewards{d}{ep}.rightWindows(:,2)]);
            wins = [trigs - timewin(1), trigs + timewin(2)];
            
            for w = 1:size(wins,1)
                win = wins(w,:);
                if wins(2) > eptime(end) %sometimes final reward was disengaged after epoch time was stopped
                    continue
                end
...
figfile = [figDir,'Specgrams\',region,'_specgram_',freqband,'_reward'];
