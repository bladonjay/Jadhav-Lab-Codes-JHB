function RunTreadmill(settings)
...
tr= Treadmill('COM3');
laserserial = serial('COM4','BaudRate',9600);
fopen(laserserial);
...
applysettings;
    ...
    function createtrialtypes()
        ...
        switch settings.lasermode
            case 'arm'
                ...
                odorlist = char((mod(trialtypes,4)>1)+'A');
    ...
    function ratresponse(event_num, event_time)
        ...
        if(object == 0)
            if(odor == 'A'); shoulddig = true;
            else shoulddig = false;
            end
        else
            if(odor == 'A'); shoulddig = false;
            else shoulddig = true;
            end
        end
    ...
    function armlaser(event_num, event_time)
        ...
        function updateCounter()
            if(ishandle(h.counter))
                ...
                % Display lap number, object, and odor.
                set(h.counter,'String',sprintf('%3d: %1d %1s %2s',lap,objectlist(lap_),odorlist(lap_),lstr));
            end
        end
