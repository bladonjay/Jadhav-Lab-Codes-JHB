function RunTreadmill2(settings)
    ...
    function createtrialtypes()
        ...
        switch settings.lasermode
            case 'arm'
                ...
                laserlist(trialtypes<11) = 0;    %0              % no laser change back for old 'arm'
                laserlist(trialtypes>=11) = 2; %2
               % laserlist(trialtypes>=16) = 2;                % arm laser
            case '5050'
                trialtypes = mod(randperm(trialblocksize),20);
                objectlist = (mod(trialtypes,2)==0);
                laserlist = (trialtypes>9);
            case 'allLaser' 
                trialtypes = mod(randperm(trialblocksize),20);
                objectlist = (mod(trialtypes,2)==0); 
                laserlist = ones(size(trialtypes));
            otherwise
                trialtypes = mod(randperm(trialblocksize),20);
                objectlist = (mod(trialtypes,2)==0);
