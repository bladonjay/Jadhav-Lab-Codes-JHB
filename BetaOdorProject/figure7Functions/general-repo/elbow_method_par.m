...
%If just_return is not specified or is empty, it's assumed to be TRUE (ie the
...
if (nargin < 4)
    horizontal=false;
elseif isempty(horizontal)
    horizontal=false;
else
    horizontal=logical(horizontal);
end
...
if ~horizontal
    ...
    bfwd=cumsum(y)./x; % mean value
    brev=flipud(cumsum(flipud(y))./x);
