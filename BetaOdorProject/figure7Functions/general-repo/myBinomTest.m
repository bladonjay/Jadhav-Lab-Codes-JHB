function pout=myBinomTest(s,n,pr,Sided)
...
%       pr-      (Scalar or Array) The proposed probability of a successful outcome
...
%
%       s, n and pr can be scalars or arrays of the same size. The
%       dimensions and size of pout will match that of these inputs.
%
%   For example, the signtest is a special case of this where the value of
%   pr is equal to 0.5 (and a 'success' is defined by whether or not a given
%   sample is of a particular sign, but the binomial test and this code is 
%   more general allowing the value of pr to be any value between 0 and 1.
%
...
if nargin<3 || isempty(pr);      pr=0.5;      end
    
s=floor(s);
[s,n,pr]= EqArrayAndScalars(s,n,pr);
...
switch lower(Sided)
    case {'two','two, equal counts'}        
        if all(pr)==0.5 && strcmpi(Sided,'two')
        ...
        if any(GreaterInds)
            pout(GreaterInds)=1-binocdf( s(GreaterInds)-1,n(GreaterInds),pr(GreaterInds));  %start with the prob of getting >= s # of successes
            ...
            if strcmpi(Sided,'two, equal counts')            
                ...
                EqInds=dE==0;
                if any(EqInds)
                    pout(EqInds)=pout(EqInds)- binopdf( E(EqInds),n(EqInds),pr(EqInds) );
                end
            else
                ...
                targy=binopdf(s(GreaterInds),n(GreaterInds),pr(GreaterInds));                                
                ...
                y=binopdf(s2,n(GreaterInds),pr(GreaterInds));
                for ii=1:length(Inds)            
                    ...
                    if y(ii) <= targy(ii)
                        ...
                        while y(ii) <= targy(ii) && s2(ii)<E(Inds(ii))
                            s2(ii)=s2(ii)+1;
                            y(ii)=binopdf(s2(ii),n(Inds(ii)),pr(Inds(ii)));
                        end
                    ...
                    if ~SkipPAdd
                        ...
                        pout(Inds(ii))=pout(Inds(ii))+ binocdf(s2(ii),n(Inds(ii)),pr(Inds(ii)));  
                    end
                end
            end                        
        end
        ...
        if any(~GreaterInds)            
            pout(~GreaterInds)=binocdf(s(~GreaterInds),n(~GreaterInds),pr(~GreaterInds));  %start with the prob of getting <= s # of successes
            ...
            if strcmpi(Sided,'two, equal counts')
                ...
                if s2<=n(~GreaterInds)
                    pout(~GreaterInds)=pout(~GreaterInds) + 1-binocdf(s2-1,n(~GreaterInds),pr(~GreaterInds));
                end
            else
                Inds=find(~GreaterInds);
                ...
                for ii=1:length(Inds)                
                    ...
                    if ~SkipPAdd
                        ...
                        pout(Inds(ii))=pout(Inds(ii))+ 1-binocdf(s2(ii)-1,n(Inds(ii)),pr(Inds(ii)));   
                    end
                end
    ...
    case 'one'  %one-sided
        if any(GreaterInds)
            pout(GreaterInds)=1-binocdf(s(GreaterInds)-1,n(GreaterInds),pr(GreaterInds));  %just report the prob of getting >= s # of successes
        end
        if any(~GreaterInds)                    
            pout(~GreaterInds)=binocdf(s(~GreaterInds),n(~GreaterInds),pr(~GreaterInds));  %just report the prob of getting <= s # of successes
        end
    otherwise
