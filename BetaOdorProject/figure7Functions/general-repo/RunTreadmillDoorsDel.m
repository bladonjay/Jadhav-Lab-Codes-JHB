function RunTreadmillDoorsDel(settings)
...
defsettings.training= true;         % not a training session
...
%% % joystick and events
    ...
    function lighton(varargin)
        ...
        output(h.maindsp, sprintf('activated quick timer'));
    end

    function lightoff(varargin)
        writeDigitalPin(myarduino,settings.lightpin,0);
        output(h.maindsp, sprintf('deactivated quick timer'));
        
    end

    % parsing events
    function joystick_event(event_num,event_time)
        % bladon map for controller, this turns the button number into
        % a variable name 
        jsmap=SNESgamepadmap;
        %jsmap = joystickmap;
        % find the name of that event number  (snes)
        ind = find(event_num == jsmap(:,1));
        if(~isempty(ind))
            % if we found a name
            event_name = jsmap(ind,2);
            process_event(event_name,'button',event_time);
        end
    end

    % responding to events
    function process_event(event_name,~,event_time)
        % process the event by name, and then use event_codes to feed its
        % code into the plx server (if we have a server to send to)
        ...
        switch event_name
            case EVENTCODES.TM % treadmill start (101)
                ...
                initTreadmill(event_name,event_time);
                if(plexserver ~= 0); PL_SendUserEvent(plexserver, event_name); end
            ...
            case {EVENTCODES.DIG, EVENTCODES.NODIG} % Responses (105 or 106)
                ratresponse(event_name, event_time)
                if(plexserver ~= 0); PL_SendUserEvent(plexserver, event_name); end
                
            case EVENTCODES.LASERZONE % 101,
                armlaser(event_name, event_time)
                if(plexserver ~= 0); PL_SendUserEvent(plexserver, event_name); end
...
%% change settings window
    function settings = changesettings(settings, h)
        ...
        if(ishandle(setwin))
            ...
            settings.rundelay = str2num(get(controls.rundelay,'String'));
