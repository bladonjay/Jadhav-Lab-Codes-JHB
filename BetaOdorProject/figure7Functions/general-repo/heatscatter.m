...
%     end
    ...
    [values, centers] = hist3([X Y], {numbins(1) numbins(2)});
    ...
    % either bins is 1 number, or its your raw vector
    if length(numbins)>1
        
        bins_X = zeros(length(numbins), 2);
        bins_Y = zeros(length(numbins), 2);
    ...
    if (plot_lsf)
        ...
        text(gca,0.14, 0.80, 1, str, 'EdgeColor', 'none','Units','normalized');
    end
