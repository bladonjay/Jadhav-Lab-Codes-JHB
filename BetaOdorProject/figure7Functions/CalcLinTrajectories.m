            ...
            if runblocks % old...
                ...
                for tr=1:length(trajinds) % for each trajectory (out left, out right, in left, in right)
                    ...
                    if sum(keepinds)>30*10 % 10 seconds of data
                        ...
                        for fn=1:length(PFfieldnames)
                            FieldProps.(PFfieldnames{fn})(tr)=FieldPropsTemp.(PFfieldnames{fn});
                        end
                        
                        % the following is the longform of the above
                        %{
                        the following is now contained in the above
                        function
