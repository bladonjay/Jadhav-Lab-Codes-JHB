...
tbin=.0025; % 2.5 milliseconds
gauss=.01; % 10 msec gaussian (so the stdev is 10msec, or about 4 bins
corrspan=.25;
        ...
        for j=1:length(PFCunits)
            ...
            for k=1:length(CA1units)
                ...
                if sum(activetrials)>3
                    ...
                    if plotIT==1
                        subplot(length(PFCunits),length(CA1units),it);
                        plot(span,squeeze(odorMatrix(j,k,:))); yyaxis right;
                        myxcorr=xcorr(Pvecnull,Cvecnull,corrspan/tbin);
                        plot(span,myxcorr);
                        it=it+1;
                    end
...
%
% now the workup of each celltype
sortwin=span>-.08 & span <.08;
sortspan=span(sortwin);
for i=1:size(corrTable,2)
    allData=[];
    for j=1:size(corrTable,1)
        thisCorr=corrTable{j,i}{:};
    ...
    xlim([span(1) span(end)]);
    subplot(3,1,3);
    [y,x]=histcounts(sortspan(b),linspace(min(sortspan),max(sortspan),25));
    plot(mean([x(1:end-1); x(2:end)]),SmoothMat2(y,[1 20],5));
     hold on; plot([0 0],get(gca,'YLim'),'k')
    xlim([min(sortspan) max(sortspan)]);
    title(sprintf('Signed-rank test p=%.2e',signrank(sortspan(b))));
end
    ...
    for j=1:length(PFCunits)
        ...
        for k=1:length(CA1units)
            ...
            if sum(badtrials)>8
                ...
                [mypeaks]=findpeaks(bootZ,2); mypeaks.loc=mypeaks.loc(abs(span(mypeaks.loc))<.05);
...
plot(mean([bins(2:end); bins(1:end-1)]),smooth(cts./max(cts)),'w-');
[~,p2]=ttest(okpairs);
...
% basically they measure the peak from +200msec to -200 msec and do a
% difference score- I think this should be thinner, like -50 to 50 msec
...
for i=1:length(SuperRat)
   ...
   odorTimes(:,1)=odorTimes(:,2)-odorDur;
        ...
        for k=1:length(CA1units)
            ...
            if sum(spikediffs<.05)>10 % if there are at least 10 spikes within 50 msec
                ...
                for td=1:length(span)
                    trCorr(td)=sum(abs(spikediffs(:)-span(td))<=tbin);
                end
                ...
                for bt=1:nboots
                    ...
                    % (0:1)-.5 will be between .5 and 5, then /10 is .05
                    ...
                    for td=1:length(span)
                        bootCorr(bt,td)=sum(abs(spikediffs-span(td))<=tbin);
                    end
                end
    ...
    corrTable.specGrams(i)={freqMatrix};
    % on first blush, it looks like specific PFC units will assume
    % peaks in their xcorr with many CA1 units during odor sampling
    % she will plot the mean of all these
    fprintf('Sess %d, rat %s %d done in %d, prob %d more\n',i,SuperRat(i).name,...
...
% get ccgram
okgrams=allGrams(:,allstats(1,:)>corrthresh & abs(allstats(2,:))<window);
[~,a]=sort(okpairs);
if Zfirst==1
...
hold on; plot([0 0],[1 length(a)],'k');
yyaxis right;
plot(span,mean(okgrams,2,'omitnan')./max(mean(okgrams,2,'omitnan')),'w-');
...
%% need to figure out how to pull 5 hz out here
%
% does any specific pair have a bump in a specific frequency range, and for
% that pair is it different from pfc first vs ca1 first.
% algorithm is... 1. get those pairs, index their peaks, then image their
% aggregate spectrograms

corrTable(cellfun(@(a) isempty(a), corrTable.xcorrgrams),:)=[];
allGrams=[]; allstats=[]; allPairs=[]; allspecGrams=[];
for i=1:height(corrTable)
    thisCorr=corrTable.xcorrgrams{i}; % get the zscored xgrams
    pairTypes=corrTable.cellTypes{i}; % 
    specGrams=corrTable.specGrams{i};

    allGrams=[allGrams reshape(permute(thisCorr,[3,1,2]),...
        [size(corrTable.xcorrgrams{1},3),size(thisCorr,1)*size(thisCorr,2)])];
    allPairs=[allPairs reshape(permute(pairTypes,[3,1,2]),...
        [size(corrTable.cellTypes{1},3),size(pairTypes,1)*size(pairTypes,2)])];
    allspecGrams=[allspecGrams reshape(permute(specGrams,[3,1,2]),...
        [size(corrTable.specGrams{1},3),size(specGrams,1)*size(specGrams,2)])];
end
...
peakwin=abs(span)<window; % ok window for peaks to be
peakTs=span(peakwin); % timestamps of those
[peakvals,peakind]=max(allGrams(peakwin,:)); % find the peak values and indices
peakTimes=peakTs(peakind); % now this is the real ts of those peaks

% plot everything, split for - and + inlaid right axis
figure;
subplot(2,2,1);
...
imagesc(span,length(theseGrams),theseGrams(:,peakinds)');
xlabel('time');
subplot(2,2,2);
%errorbar(freqs,mean(allspecGrams(:,okcells),2).*freqs',SEM(allspecGrams(:,okcells),2).*freqs');
plot(span,mean(theseGrams,2,'omitnan'))
xlabel('frequency'); ylabel('power (dB)')
subplot(2,2,3);

theseSpec=zscore(allspecGrams(:,okcells));
[~,peakinds]=max(theseSpec);
[~,reordered]=sort(peakinds);
...
for i=1:4
    ...
    corrthresh=2;
    okcells=peakvals>corrthresh & allPairs==i-1;
    [~,peakinds]=sort(peakTimes(okcells));
    theseGrams=allGrams(:,okcells);
    imagesc(span,length(theseGrams),theseGrams(:,peakinds)');
    xlabel('time'); title('xcorrgrams')
    subplot(2,2,2);
    plot(span,mean(theseGrams,2,'omitnan'))
    xlabel('frequency'); ylabel('power (dB)')
    subplot(2,2,3);
    ...
    errorbar(freqs,mean(zscore(theseSpec(:,peakTimes(okcells)<0)),2).*freqs',SEM(zscore(theseSpec(:,peakTimes(okcells)<0)),2).*freqs');
    hold on;
    errorbar(freqs,mean(zscore(theseSpec(:,peakTimes(okcells)>0)),2).*freqs',SEM(zscore(theseSpec(:,peakTimes(okcells)>0)),2).*freqs');    xlabel('frequency'); ylabel('power (dB)');
    xlabel('frequency'); ylabel('power (dB)'); legend({'PFC leads pair','CA1 leads pair'})
