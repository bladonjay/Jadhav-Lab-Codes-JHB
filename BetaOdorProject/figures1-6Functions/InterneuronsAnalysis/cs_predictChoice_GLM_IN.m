...
function [fract_correct, fract_correct_shuff] = cs_predictChoice_GLM_IN(animal, day, region, window,celltype)
topDir = cs_setPaths();
...
switch celltype
    case 'selective'
        filt = ['strcmp($area,''',region,''') && $numspikes>0 && strcmp($type,''int'') && (strcmp($selectivity,''rightSelective'') || strcmp($selectivity,''leftSelective''))'];
    case 'all'
        filt = ['strcmp($area,''',region,''') && $numspikes>0 && strcmp($type,''int'')'];
end
