...
for f = 1:length(freqbands)
    ...
    load([dataDir, 'selectiveINs_',region,'.mat']);
    ...
    bothCA1 = intersect(selectiveint,plcells,'rows');
    
    fractionselective = size(selectiveint,1) / size(npInt,1);
    fractionpl = size(plcells,1)/size(npInt,1);
    fractionboth = size(bothCA1,1)/size(npInt,1);
    
    probbothCA1 = (fractionpl*fractionselective)*100;
    %probbothCA1 = fractionselective*fractionpl*100;
    pboth_CA1 = binopdf(size(bothCA1,1),size(npInt,1),(fractionpl*fractionselective));
    
    barsCA1 = [fractionselective,fractionpl,fractionboth];
    barsCA1 = barsCA1*100; %percentage
    
    X = binoinv([0.05 0.95],size(npInt,1),(fractionpl*fractionselective));
    CI_CA1 = X/size(npInt,1)*100;
    ...
    bothPFC = intersect(selectiveint,plcells,'rows');
    
    fractionselective = size(selectiveint,1) / size(npInt,1);
    fractionpl = size(plcells,1)/size(npInt,1);
    fractionboth = size(bothPFC,1)/size(npInt,1);
    
    barsPFC = [fractionselective,fractionpl,fractionboth];
    barsPFC = barsPFC*100; %percentage
    
%     probsel = binofit(size(selectiveint,1),size(npInt,1),0.5);
%     probpl = binofit(size(plcells,1),size(npInt,1),0.5);
%     probbothPFC = binofit(size(bothPFC,1),size(npInt,1),(probpl*probsel));
    probbothPFC = (fractionpl*fractionselective)*100;
    %probbothCA1 = fractionselective*fractionpl*100;
    pboth_PFC = binopdf(size(bothPFC,1),size(npInt,1),(fractionpl*fractionselective));
    %probbothPFC = fractionselective*fractionpl*100;
    X = binoinv([0.05 0.95],size(npInt,1),(fractionpl*fractionselective));
    CI_PFC = X/size(npInt,1)*100;
    ...
    box off
    ylabel('Percentage of Interneurons');
    xticklabels({'CA1','PFC'})
    axis([0.5 2.5, 0 100])
    hold on
    ...
    figfile = [figDir,'SelectivityPLOverlap_IN_',freqband];
    
    legend({'Selective Int',[freqband, ' Phase Locked Int'],'Overlap'});
