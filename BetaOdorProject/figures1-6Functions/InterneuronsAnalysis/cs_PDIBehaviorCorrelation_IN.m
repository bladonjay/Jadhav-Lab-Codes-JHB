...
for r = 1:length(regions)
    ...
    for a = 1:length(animals)
        ...
        files = dir([animaldir,animal,'trialSigPDI_NP_IN_',region,'*']);
...
figtitle = ['PVDivergence-NPlength_raster_IN_',region];
...
figtitle = ['PVDivergence-NPlength_correlation_IN_',region];
figfile = [figDir,'PopSelectivity\',figtitle];
%saveas(gcf,figfile,'fig');
print('-dpdf', figfile);
print('-djpeg', figfile);
...
[means] = cs_sextiles(sortedNP,sortedPDI);
 ...
 figtitle = ['PVDivergence-NPlength_sextile_IN_',region];
