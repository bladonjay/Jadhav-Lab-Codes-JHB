...
function [fract_correct, fract_correct_shuff] = cs_predictOutcome_GLM(animal, day, region, window)
topDir = cs_setPaths();
...
for c = 1:numcells %get spikes for each cell
    ...
    if ~isempty(intersect(goodeps, eps)) %cell should spike during at least one run epoch
        for ep = eps'
            ...
            spikebins = periodAssign(spiketimes, trials(:,[1 2])); %Assign spikes to align with each trials(same number = same trial, number indicates trial)
            
            spikebins = nonzeros(spikebins);
...
%Throw out cells here that are not active on at least 10 trials
activetrials = sum(Cellmatrix,1);
validcells = activetrials>= 10;
Cellmatrix = Cellmatrix(:,validcells);

%If fewer than 5 cells, don't try GLM 
