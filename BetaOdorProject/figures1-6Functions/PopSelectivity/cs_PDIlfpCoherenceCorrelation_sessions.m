                ...
                if ~isempty(files)
                    for f = 1:length(files)
                        ...
                        for e = 1:length(epochs)
                            ...
                            keepinds = ~isnan(divTimes);
                            
                            if sum(keepinds) > 0
                            bp = bp(keepinds);
                            divTimes = divTimes(keepinds);
                            
                            allLFPCoh = [allLFPCoh; mean(bp)];
                            allDivTimes = [allDivTimes; mean(divTimes)];
...
%             end
    ...
    figtitle = ['PVDivergence_',freq,'Coherence_sextile'];
