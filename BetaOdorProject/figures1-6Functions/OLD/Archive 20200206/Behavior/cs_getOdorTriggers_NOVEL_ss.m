function cs_getOdorTriggers_NOVEL_ss(prefix,topDir, days,epochs)
...
dayfolders = dir() ;
    ...
    cd(dayfolders{day})
        ...
        for i = 1:length(ssfiles)
            ...
            k = strfind(epochname,'Sleep');
            if isempty(k)
                runEps = [runEps; epochname];
            end
        end
...
%         cd([dataDir,'OdorTriggers']);
        save([prefix, 'novelOdorTriggers', daystring],'odorTriggers');
        
%         if ~exist([dataDir,'BinaryPerf'])
%             mkdir([dataDir,'BinaryPerf'])
%         end
%         cd([dataDir,'BinaryPerf']);
        save(['NovelBinaryPerf',daystring,'-', epstring],'CorrectIncorrectBinary');
