...
for r = 1:length(regions)
    ...
    %nonselectivecells = setdiff(npCells,selectivecells,'rows');
    spinfo_s = [];
    spinfo_n = [];
    for a = [1 2 3 4 8]
        animal = animals{a};
        ...
        nonselectivecells = setdiff(animCells,selcells,'rows');
        
        %animCells = npCells(npCells(:,1) == a,2:4);
        
        days = unique([animCells(:,1); animCells(:,1)]);
        ...
        for d = days'
            ...
            for s = 1:size(dayCells,1)
                ...
                elseif ismember([d, cell], nonselectivecells, 'rows')
