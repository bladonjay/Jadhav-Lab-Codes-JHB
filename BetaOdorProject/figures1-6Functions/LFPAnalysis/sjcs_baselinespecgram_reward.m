function [params] = sjcs_baselinespecgram_reward(prefix, days, epochs, tets, do_wrtgnd,varargin)
% Shantanu - Nov 2012.
...
% new version - uses reward as baseline rather than whole epoch
...
for d=1:length(days)
    ...
    % get reward windows to use as baseline
    % -------------------------------------------
    rewards = loaddatastruct(directoryname, prefix,'rewards',day);
    ...
    for t=1:length(tets)
        ...
        for e=1:length(epochs)
            ...
            if do_wrtgnd==1
                ...
                if (exist([curreeggndfile,'.mat'],'file'))==2
                    disp(['Doing ', prefix,' Day ',num2str(day) ', Ep ',num2str(ep),', Tet ',num2str(tet)]);
                    load(curreeggndfile);
                    
                    starttime = eeg{day}{ep}{tet}.starttime;
                    endtime = eeg{day}{ep}{tet}.endtime;
                    rtimes = rtimes-starttime;
                    
                    %Remove triggering events that are too close to the end
                    bad = rtimes+starttime + 2 > endtime;
                    rtimes(bad) = [];
                    
                    [specgram_gnd, ~, Sfreq] = mtspecgramtrigc(eeg{day}{ep}{tet}.data,rtimes(:,1),[0 2],movingwin,params);
                ...
                if exist([prefix,'eegref',daystring,'-',epstring,'-',tetstring,'.mat']) == 2
                    disp(['Doing Day ',num2str(day) ', Ep ',num2str(ep),', Tet ',num2str(tet)]);
                    ...
                    specgram = mtspecgramc(eegref{day}{ep}{tet}.data,movingwin,params);
                    % Dont save specgram in file - its too big
                    % --------------------------------------------
        ...
        if do_wrtgnd==1
            ...
            eeggndspec_reward{day}{1}{tet}.meandayspec=mean(dummy_gnd,1);
            eeggndspec_reward{day}{1}{tet}.stddayspec=std(dummy_gnd,1);
            eeggndspec_reward{day}{1}{tet}.freq = Sfreq;
            % Save File for current day and tet
            savefile = [savedir,prefix,'eeggndspec_reward',savetag,daystring,'-',tetstring];
            save(savefile,'eeggndspec_reward');
            clear eeggndspec_reward
        else
