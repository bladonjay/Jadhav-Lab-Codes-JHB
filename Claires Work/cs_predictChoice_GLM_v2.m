function [fract_correct, fract_correct_shuff] = cs_predictChoice_GLM_v2(animal, day, window,cells)
topDir = cs_setPaths();
...
%     case 'all'
%         filt = ['(strcmp($area,''CA1'') || strcmp($area,''PFC'')) && $numspikes>0 && strcmp($type,''pyr'')'];
% end


% cells = evaluatefilter(cellinfo{day},filt);
cells = unique(cells(:,[3 4]),'rows');
numcells = size(cells,1);

% gettrial times and ids
...
for c = 1:numcells %get spikes for each cell
    ...
    if ~isempty(intersect(goodeps, eps)) % cell should spike during at least one run epoch
        for ep = eps'
            ...
            trials(:,2) = trials(:,1) + window; % this is 0.8 for me
            ...
            % Assign spikes to a trial (same number = same trial, number indicates trial)
            spiketrials = periodAssign(spiketimes, trials(:,[1 2]));  % assign each spike an event it lies within
            spiketrials = nonzeros(spiketrials); % guessing this removes spieks not assigned to a trial
            spikecount = zeros(1,size(trials,1)); % preallocate variable
            % this is an analog version of accumarray
            for s = 1:length(spiketrials)
                spikecount(spiketrials(s)) = spikecount(spiketrials(s))+1;
            end
            % basically this gets the number of spikes in each timebin
            % (odor period)
            ...
            fulltrials = [trials(:,1), trials(:,1) + fullwin]; % this is for a longer window
